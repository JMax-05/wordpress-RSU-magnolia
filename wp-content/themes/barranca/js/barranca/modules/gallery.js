...
export function initGallery() {
  ...
  const boxes = galleryImages.map((image, index) => {
    ...
    box.setAttribute('aria-label', `Fotografia ${index + 1} de Barranca`);
  ...
  const playBoxes = () => {
    boxes.forEach((box) => {
      ...
      gsap.to(box.galleryTimeline, { duration: 0.4, timeScale: 1, ease: 'sine.in', overwrite: true });
    });
  };
  ...
  const handleHoverIn = (box) => {
    ...
    pauseColumn(box);
    gsap.to(boxes, { duration: 0.2, overwrite: 'auto', opacity: (_, target) => (target === box ? 1 : 0.33) });
